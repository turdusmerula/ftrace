...
//-----------------------------------------------------------------------------
__thread std::map<void*,  Scope*>* Scope::_threadScopeList=NULL ;
__thread std::stack<Scope*>* Scope::_threadScopeStack=NULL ;
__thread Scope* Scope::_threadCurrScope=NULL ;
__thread int Scope::_threadTabNum=0 ;
__thread Scope::MemStats Scope::_threadMemStats ;
...
//-----------------------------------------------------------------------------
std::vector< std::pair<void*,  Scope*> >::iterator Scope::find(std::vector< std::pair<void*,  Scope*> >& list_, void* func_, std::vector< std::pair<void*,  Scope*> >::iterator* istart_)
{
    //TODO: optimize this function

    /*std::vector< std::pair<void*,  ScopeData*> >::iterator iup ;
    std::vector< std::pair<void*,  ScopeData*> >::iterator idown ;
    ...
    {
        //The up iterator search from the given iterator to the end, the down from the given iterator to the start
        iup = *istart_ ;
        idown = *istart_ ;
    }

    while(iup!=list_.end() && idown!=list_.begin())
    {
        if((*iup).first==func_)
            return iup ;
        if((*idown).first==func_)
            return idown ;

        if(iup!=list_.end())
            iup++ ;
        if(idown!=list_.begin())
            idown-- ;
    }

    return list_.end() ;*/
    std::vector< std::pair<void*,  Scope*> >::iterator iup ;
